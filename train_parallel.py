...
if __name__ == "__main__":
    ...
    episodes_to_run = 500; batch_size = 64; log_file_path_base = 'training_log_parallel.csv'
    
    # --- Path Setup (Handles Colab vs. Local) ---
    ...
    while episodes_completed < episodes_to_run:
        ...
        if isinstance(experience[0], str) and experience[0] == 'episode_done':
            ...
            with open(log_file_path, 'a', newline='') as file:
                writer = csv.writer(file); writer.writerow([episodes_completed, stats['reward'], stats['timesteps']])
            ...
            if episodes_completed > 0 and episodes_completed % 10 == 0:
                main_agent.save(os.path.join(model_dir, f"traffic-model-e{episodes_completed}.weights.h5"))
            if episodes_completed > 0 and episodes_completed % 50 == 0:
        ...
        if len(main_agent.memory) > batch_size:
            ...
            if learning_steps % 200 == 0: print(f".", end='', flush=True)
